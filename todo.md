...
- [x] **10.2 Invoice PDFs** (Session 029) — WeasyPrint 63.1 + Docker native deps; `Invoice` model (`INV-YYYYMMDD-NNNNN`, FK Booking PROTECT, type, amount, snapshot fields frozen at issue time); self-contained `invoices/invoice_pdf.html` letterhead; bank/M-Pesa payment details now on the **dashboard SiteSettings page** (not `.env`); staff-only issue/download/email actions on booking detail; `send_invoice_email()` in `packages/emails.py`; `humanize` for money formatting. 15 tests, live-verified end-to-end.
